...
│   ├── user/               # Perfil de usuario (GET /user/profile)
│   ├── maintenance/        # Estado de módulos (GET/POST /modules)
│   ├── login/              # Autenticación (POST /auth/login)
│   └── [feature]/          # Otros servicios por dominio
...
### Flujo de Autenticación (`useAuth`)

`useAuth` valida la expiración del token con `jwtDecode` y obtiene los datos del perfil
desde el backend. El usuario nunca se construye a partir del payload del JWT.

```typescript
// Entidad User — refleja la respuesta real de GET /user/profile
...
// useAuth: jwtDecode solo verifica exp, el perfil viene del servidor
const { data: userData } = useQuery({
  queryKey: ['userProfile'],
  queryFn: getUserProfile,   // GET /user/profile
  enabled: isTokenValid(),
  staleTime: 5 * 60 * 1000,
});
```

**Tests**: cualquier hook o componente que llame `useAuth` necesita `QueryClientProvider`
y mockear `@/services/user/user.service`:
...
### Patrón reCAPTCHA en formularios

`useLoginForm` y `useContactForm` bloquean el submit con `if (isVerified) return`.
`isVerified === true` significa reCAPTCHA pendiente; `false` significa verificado.

```typescript
// En tests de submit, mockear useRecaptcha con isVerified: false
vi.mock('@/hooks/useRecaptcha');
vi.mocked(useRecaptcha).mockReturnValue({
  recaptchaRef: { current: null } as any,
  isVerified: false,   // false = verificado, el submit no hace early return
